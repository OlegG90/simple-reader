...
fn register_in(classes: &RegKey, exe: &Path) -> io::Result<()> {
    ...
    for ext in EXTENSIONS {
        ...
        // Become the default only where no default is set for this user.
...
mod tests {
    ...
    fn register_adds_open_with_and_default_then_unregister_removes_them() {
        ...
        register_in(&classes, Path::new(EXE)).unwrap();
